...
class FeedbackDetailType {
    ...
    public $ItemID;
    /**
     * @var \Controle\TradingRoleCodeType | Indicates whether the user who was the feedback recipient was a Buyer or the Seller
     ...
     * must go through a verification process before the feedback they leave can be counted
     * in the score. Whether or not a feedback is countable is determined by the verification
     * status of the person leaving the feedback at the time the feedback is left. <br><br> If
     * a user is not verified and leaves a feedback, then later becomes verified, that feedback
     * will still not be counted. Or, if a user is verified and at some later date changes
     * to unverified status, the feedback left while the user was verified remains countable.
     * So you cannot determine a user's current verification status from whether or not feedback
     * they've left is or was countable. Whether or not feedback is countable has nothing to
    ...
    public function setFeedbackResponse($val) {
        if(!is_string($val)) throw new Exception('POJO Proxy need a string for FeedbackResponse');
    ...
    public function setFollowup($val) {
        if(!is_string($val)) throw new Exception('POJO Proxy need a string for Followup');
    ...
    public function setItemTitle($val) {
        if(!is_string($val)) throw new Exception('POJO Proxy need a string for ItemTitle');
