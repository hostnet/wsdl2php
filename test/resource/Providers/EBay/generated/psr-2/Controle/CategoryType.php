...
class CategoryType
{
     ...
     * Item Information feature uses the Item Specifics feature, the set of catalog-enabled
     ...
     * in the site's category hierarchy. For the GetSearchResults response, NumOfItems indicates
     ...
     * is 1. For GetSuggestedCategories, multiple CategoryParentID fields can be returned in
     * sequence, starting with the root category and ending with the category that is the direct
     * parent of the category specified in CategoryID. Use these parent fields and the CategoryID
    ...
    /**
     * @var \Controle\ExtendedProductFinderIDType | Returned from GetCategory2CS calls with compatibility level 369 and higher. Supports
    ...
    /**
     * @var \Controle\CharacteristicsSetType | Returned from GetCategory2CS calls. A list of one or more characteristics sets mapped
     ...
     * value. That is, if the request specifies that fewer categories or subcategories should
     ...
     * (override reserve price allowed) indicates when the category is an exception to the
     * site's ReservePriceAllowed policy.<br> <br> If ORPA is true, the category overrides
     * (toggles or reverses) the site's ReservePriceAllowed setting. In other words:<br> -
     * If ReservePriceAllowed is true, reserve price is not allowed in this category.<br> -
     * If ReservePriceAllowed is false, reserve price is allowed in this category.<br><br> If
     * ORPA is not present (or false), there is no override. That is, the category's setting
     * is the same as the site's ReservePriceAllowed setting.<br> <br> This toggling logic
     * is designed to reduce the size of the GetCategories response by only returning ORPA
     * when the category's policy is different from the site's policy. (If ORPA is true for
     ...
     * by ReduceReserveAllowed. ORRA (override reduce reserve allowed) indicates when the category
     * is an exception to the site's ReduceReserveAllowed policy.<br><br> If ORRA is true,
     * the category overrides (toggles or reverses) the site's ReduceReserveAllowed setting.
     * In other words:<br> - If ReduceReserveAllowed is true, reserve price reduction is not
     * allowed in this category.<br> - If ReduceReserveAllowed is false, reserve price reduction
     * is allowed in this category.<br><br> If ORRA is not present (false), there is no override. That
     * is, the category's setting is the same as the site's ReduceReserveAllowed setting.<br> <br> This
     * toggling logic is designed to reduce the size of the GetCategories response by only
     * returning ORRA when the category's policy is different from the site's policy.  (If
