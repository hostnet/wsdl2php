...
class SiteHostedPictureType
{
    ...
    public $PictureURL;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\PhotoDisplayCodeType | Specifies the type of image display used in a listing. Some options are only available
    ...
    public $PhotoDisplay;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\GalleryTypeCodeType | Indicates whether a picture for the item is included in the Gallery and, if so whether
     * the item is also featured or Plus in the Gallery. If the value is Gallery or Plus or
     * Featured, eBay checks the available image URLs in the following order to determine which
     * URL to use (if more than one of these URLs is available):<br><br>            a) Use
     * the URL in GalleryURL, if it is specified.<br>            b) Otherwise, if ProductListingDetails.UseStockPhotoURLAsGallery
     * is true, use the eBay stock photo.<br>            c) Otherwise, use the value of the
     * first SelfHostedURL in the array of SelfHostedURL fields, if any.<br>            d)
     * Otherwise, if ProductListingDetails.ProductID is specified, use the eBay stock photo
    ...
    public $GalleryType;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\anyURI | URL for a picture to be used as the Gallery thumbnail. Ignored if GalleryType is None
     ...
     * formats JPEG, BMP, TIF, or GIF. eBay Live Auctions lots are listed in the gallery for
     * free. With the future deprecation of VendorHostedPicture as input, Applications should
     * use Item.PictureDetails.GalleryURL instead. If the item has bids (or items have been
     * sold) or the listing ends within 12 hours, you can add GalleryURL or change its value,
     * but you cannot remove the gallery picture if it was previously specified. Not applicable to
     * eBay Motors listings. See "Describing Items in a Listing" in the If the value of GalleryType
     * is Gallery or Plus or Featured, see eBay Web Services Guide for additional validation
    ...
    public $GalleryURL;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\PictureSourceCodeType | The service hosting the images in PictureURL, if any. This information is primarily
