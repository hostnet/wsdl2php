...
class AccountSummaryType
{
    ...
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AccountStateCodeType | Indicates the current state of the account (such as active or inactive). Possible values
     * are enumerated in the AccountStateCodeType code list.
     */
    public $AccountState;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AmountType | Specifies payment made since the previous invoice, but is returned only if AccountHistorySelection
    ...
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AmountType | Specifies credits granted since the previous invoice, but is only returned AccountHistorySelection
     * is LastInvoice or Specified Invoice. With compatibility level 387, the value is positive
     * for debits and negative for credits, reversing the sign of earlier versions.
     */
    public $InvoiceCredit;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AmountType | Specifies fees incurred since the last invoice, including tax if applicable. Returned
     * only if AccountHistorySelection is LastInvoice or Specified Invoice. With compatibility
     * level 387, the value is positive for debits and negative for credits, reversing the
     * sign of earlier versions.
     */
    public $InvoiceNewFee;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AdditionalAccountType | Contains the data for one additional account. An AccountSummaryType object
    ...
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AmountType | Amount past due, 0.00 if not past due. With compatibility level 387, the value is positive
     * for debits and negative for credits, reversing the sign of earlier versions.
     */
    public $AmountPastDue;
    /**
     * @var string | First four digits of the bank account the user associated with the seller account (with
     * remainder Xed-out). This may be an empty string depending upon the payment type the
     * user selected for the account (e.g., if no debit-card specified).
     */
    public $BankAccountInfo;
    /**
     * @var dateTime | Indicates the date and time BankAccountInfo was last modified, in GMT. (Also see the
     * Data Types appendix for more information on how GMT dates are handled in SOAP.) This
     * may be an empty string depending upon the payment type the user selected for the account
     ...
     * in GMT. (Also see the Data Types appendix for more information on how GMT dates are
     * handled in SOAP.) Empty string if no credit card is on file or if the account is inactive
     * - even if there is a credit card on file.
     */
     ...
     * in GMT. (Also see the Data Types appendix for more information on how GMT dates are
     * handled in SOAP.) This may be an empty string depending on the payment method the user
     * selected for the account (e.g., Empty string if no credit card is on file.)
     */
    public $CreditCardModifyDate;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AmountType | Current balance for the account. Can be 0.00, positive, or negative. With compatibility
     * level 387, the value is positive for debits and negative for credits, reversing the
     * sign of earlier versions.
     */
    ...
    public $Email;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AmountType | Amount of last invoice. 0.00 if account not yet invoiced. With compatibility level 387,
    ...
    public $InvoiceDate;
    /**
     * @var \Controle\Hostnet\Namesp\Sub4\Complex\AmountType | Amount of last payment posted, 0.00 if no payments posted. With compatibility level
     * 387, the value is positive for debits and negative for credits, reversing the sign of
     * earlier versions.
     */
    public $LastAmountPaid;
    /**
     * @var dateTime | Date of last payment by the user to eBay, in GMT. (Also see the Data Types appendix
     * for more information on how GMT dates are handled in SOAP.) Empty string if no payments
     * have been posted.
     */
